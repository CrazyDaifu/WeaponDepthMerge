...
The script rejects `-PublishRelease` while `VERSION` contains a pre-release suffix such as `1.1-rc6`, or when the requested tag does not exactly match `v<VERSION>`.
...
For 1.1, do not run the publishing script with `-PublishRelease` until the `releases/1.1-rc6/` candidate passes native D3D9 and DXVK startup plus repeated Alt+Tab testing.
